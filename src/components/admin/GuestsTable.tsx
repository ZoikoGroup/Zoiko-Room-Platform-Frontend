    ...
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-6">
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((g, i) => (
          <div
            key={g.id}
            className="animate-fade-up rounded-2xl p-4 ring-1 ring-slate-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
            style={{ animationDelay: `${Math.min(i, 8) * 0.04}s` }}
          >
            <div className="flex items-center gap-3">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={g.avatar} alt={g.name} className="h-12 w-12 rounded-full bg-primary-50" />
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-primary-900">{g.name}</p>
                <p className="truncate text-xs text-slate-400">{g.location}</p>
              </div>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
              <div>
                <p className="font-bold text-primary-900">{g.totalBookings}</p>
                <p className="text-slate-400">Bookings</p>
              </div>
              <div>
                <p className="font-bold text-primary-900">{formatCurrency(g.totalSpent)}</p>
                <p className="text-slate-400">Total Spent</p>
              </div>
              <div>
                <p className="font-bold text-primary-900">{formatDate(g.joinedAt)}</p>
      ...
      {filtered.length === 0 && (
        <p className="mt-10 text-center text-sm text-slate-400">No guests match your search.</p>
      )}
    </div>
  );
}
